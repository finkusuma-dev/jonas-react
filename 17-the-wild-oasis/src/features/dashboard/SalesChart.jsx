...
function SalesChart({ bookings, numDays }) {
  ...
  //   startOfDay(new Date(bookings[0].created_at)),
  //   startOfDay(allDates[2]),
  //   isSameDay(new Date(bookings[0].created_at), allDates[2]),
  //   differenceInCalendarDays(new Date(bookings[0].created_at), allDates[2])
  // );

  const data = allDates.map((date) => ({
    label: format(date, 'MMM dd'),
    totalSales: bookings.reduce(
      (acc, item) =>
        isSameDay(new Date(item.created_at), date)
          ? acc + item.total_price
          : acc,
      0
    ),
    extrasSales: bookings.reduce(
      (acc, item) =>
        isSameDay(new Date(item.created_at), date)
          ? acc + item.extras_price
          : acc,
      0
    ),
  }));
